...
wsServer.on('request', function(request) {
    ...
    var chatRoom, connection = request.accept(null, request.origin); 
    ...
    var index = chatRoom.clients.push(connection) - 1;
    var userName = false;
    var userColor = false;
    ...
    connection.on('message', function(message) {
        if (message.type === 'utf8') { // accept only text
            if (userName === false) { // first message sent by user is their name
                ...
                for (var i=0; i < chatRoom.clients.length; i++) {
                    chatRoom.clients[i].sendUTF(json);
                }
            }
        }
    });
    ...
    connection.on('close', function(connection) {
        if (userName !== false && userColor !== false) {
            ...
            chatRoom.clients.splice(index, 1);
